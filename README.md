...
- Let visitors reserve places and download their ticket references without overbooking an event.
- Give organisers a place to manage events, post notices and check tickets.
...
- Search events and filter them by category on desktop or mobile.
- Shareable event details (`/?event=EVENT_ID`) with date, time, venue and live capacity.
- Monthly calendar with tappable dates on mobile, event lists and calendar-file downloads.
- Spotlight and countdown for the next published event. Started and cancelled events are skipped; the programme refreshes every minute.
- Registrations stored in SQLite, with one booking per email per event and a capacity check that handles simultaneous requests.
...
- Clear messages when there are no events, an event is full or a request fails.
...
2. Create the production database:
   ...
   ```
3. Copy the returned database ID into `wrangler.jsonc`, replacing the existing `database_id`. Keep the binding name `DB`. You may change the Worker name as well.
...
8. For a separate deployment, update the live URL in this README. Test browsing, registration, ticket lookup and organiser check-in before submitting.

Database IDs identify a binding; they are not credentials. Never commit account tokens, private keys, `.env` or `.dev.vars` files. The repository contains the database schema and sample events, not live bookings. A new database starts with the example programme.
...
- `lib/events.ts`: event types, sample events and date formatting.
- `lib/spotlight.ts`: selects the next published event for the spotlight and countdown.
...
All event times are Sri Lanka time (UTC+05:30). Names and emails are stored for ticket administration. Ticket UUIDs act as high-entropy booking references and must be kept private. Bookings are stored on the server, not in browser storage. SQL parameters prevent injection; capacity is checked in the same SQL statement that reserves the place. Cancelled and checked-in tickets cannot be used for admission. Both confirmed and checked-in tickets consume capacity. Every organiser mutation checks the server-side allowlist and rejects cross-origin browser requests.
